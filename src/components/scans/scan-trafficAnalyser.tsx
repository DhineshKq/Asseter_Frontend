...
export default function ScanTrafficAnalyserPage({ setscanPageView }: propsType) {
    ...
    useEffect(() => {
        ...
        newSocket.on("traffic-done", (exitCode: number) => {
            ...
            if (!scanOutputRef.current.includes("Stopping Analyser")) {
                setShowMessage(exitCode === 0 ? "Traffic analysis completed." : `Scan failed with code ${exitCode}`);
        ...
        newSocket.on("disconnect", () => {
            setShowMessage("Disconnected from traffic analyser.");
            setShowType("danger");
            setShowAlertBox(true);
            clearAlert();
        });
    ...
    async function startTrafficScan(scanId: any) {
        ...
        try {
            ...
            console.error("Error starting traffic analyser:", error);
            setShowMessage(error.response?.data?.message || error.message);
            setShowType("danger");
            setShowAlertBox(true);
            setIsLoading(false);
    ...
    async function stopTrafficScan() {
        try {
            ...
            setIsLoading(false);
            setIsRunning(false);
            setShowMessage("Traffic analyser stopped.");
            setShowType("success");
            setShowAlertBox(true);
            clearAlert();
        } catch (error) {
            console.error("Error stopping traffic analyser:", error);
        }
    }
        ...
        <div className="scans-main-page">
            <div className="pageHeader">
                <div className="Breadcrumb">
                    ...
                    <h2 className='pageHeading' style={{ marginLeft: "10px" }}>{">"}</h2>
                    <h2 className='pageHeading' style={{ marginLeft: "10px" }}>{"Traffic Analysis"}</h2>
                </div>
            </div>
