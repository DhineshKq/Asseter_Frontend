...
const AgGrid = forwardRef(({
    ...
    const [pagesize, setPagesize] = useState(pagesizeProp ? pagesizeProp : 10);
    let value = {
    ...
    //     return (params.data?.voyageDetails && params.data.voyageDetails.length > 0) ? (params.data.voyageDetails.length * (rowHeight || 1)) : rowHeight ? rowHeight : 40
    // };

    const getRowHeight = (params: any) => {
        let length = 0;
        let dynamicRowHeight: any = 0;
        if (gridName === "Operation") {
            params.data?.VesselVoiage && params.data?.VesselVoiage.map((data: any) => {
                length += data.voyageDetails.length ? data.voyageDetails.length : 1
            })
            // dynamicRowHeight = (length * (rowHeight || 1)) ? rowHeight : 40
            dynamicRowHeight = (length * (rowHeight || 1)) ? (length * (rowHeight || 1)) : rowHeight
        } else if (gridName === "cargoManagement") {

            // params.data?.shipmentDetails && params.data?.shipmentDetails.map((data: any) => {
            //     console.log("first", data)
            //     length += data.shipmentDetails.length ? data.shipmentDetails.length : 1
            // })
            // dynamicRowHeight = rowHeight ? rowHeight : 40
            if (params.data?.shipmentDetails?.length * 100 > 200) {
                dynamicRowHeight = (params.data?.shipmentDetails?.length) * 100 

            }else{
                dynamicRowHeight = rowHeight || 40
            }
            // dynamicRowHeight = ((params.data?.shipmentDetails?.length) * 100 + (rowHeight || 1)) ? ((params.data?.shipmentDetails?.length) * 100 + (rowHeight || 1)) : rowHeight
        } else {
            dynamicRowHeight = rowHeight ? rowHeight : 40
        }
        // return (params.data?.VesselVoiage && params.data.VesselVoiage.length > 0) ? (length * (rowHeight || 1)) : rowHeight ? rowHeight : 40
    ...
    return (
        <div style={containerStyle} className='aggrid-component' data-testid={"column-header"}>
            ...
            {/* ag-no-record-image */}
            <div className={tabelRowData && tabelRowData.length === 0 ? "ag-theme-alpine" : "ag-theme-alpine"}>
                <AgGridReact
                    ref={gridRef}
                    ...
                    pagination={true}
                    paginationPageSizeSelector={paginationPageSizeSelector ? paginationPageSizeSelector : [10, 20, 50, 100]}
